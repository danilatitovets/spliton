// Release navigation stress: ~100 navigations against a live base URL.
...
async function countApis(page, path) {
  const seen = [];
  const handler = (req) => {
    const u = req.url();
...
async function main() {
  ...
  for (let i = 0; i < actions.length; i++) {
    ...
    try {
      ...
      timings.push({ i, type: a.type, path, ms: Date.now() - t0, status: 0, error: e.message });
      continue;
    }
    const ms = Date.now() - t0;
    const row = { i, type: a.type, path, ms, status };
    timings.push(row);
    if (path.includes("/catalog") && !path.includes("/buy")) catalogSamples.push(row);
    if (path.includes("/analytics/releases/")) releaseSamples.push(row);
    if (path.includes("/secondary-market")) marketSamples.push(row);
    if ((i + 1) % 10 === 0) console.log(`nav ${i + 1}/${actions.length} last=${ms}ms ${path}`);
  }
  ...
  for (const p of ["/app", "/catalog", "/dashboard/secondary-market", "/login"]) {
    ...
    mobile.push({ path: p, ms: Date.now() - t0, status: r?.status() ?? 0 });
  }
  ...
  writeFileSync(resolve("PERFORMANCE_NAV_STRESS.json"), JSON.stringify(report, null, 2));
