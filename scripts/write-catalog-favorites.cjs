...
const path = "apps/frontend/features/catalog/hooks/use-catalog-favorites.ts";
...
export function useCatalogFavorites() {
  ...
  const hydrate = useCallback(async (opts?: { soft?: boolean }) => {
    ...
    try {
      ...
      // Keep current favorites on transient errors (token refresh races, etc.)
      if (gen !== hydrateGen.current) return;
    } finally {
  ...
  // Hydrate only when auth settles / auth flag changes — not on every token refresh.
  useEffect(() => {
...
fs.writeFileSync(path, content.replace(/\\`/g, "`").replace(/\\\$\{/g, "${"), "utf8");
console.log("ok", !fs.readFileSync(path).includes(0));
