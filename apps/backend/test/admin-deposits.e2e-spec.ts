...
function staffEmail(prefix: string): string {
  return `e2e-deposits-${prefix}-${Date.now()}@example.com`;
}
...
async function registerUser(app: E2eApp, email: string) {
  ...
  const reg = await request(app.getHttpServer())
    .post('/auth/register')
      .send(e2eRegisterPayload(email, password, 'E2E Deposits'));
...
describe('Admin deposits API (e2e)', () => {
  ...
  it('GET /api/admin/v1/deposits without session returns 401', async () => {
    const res = await request(app!.getHttpServer()).get(
      '/api/admin/v1/deposits',
    );
    expect(res.status).toBe(401);
  });

  it('ACCOUNTANT can view deposits summary', async () => {
    const token = await staffToken(app!, UserRoleCode.ACCOUNTANT);
    const res = await request(app!.getHttpServer())
      .get('/api/admin/v1/deposits/summary')
      .set('Authorization', `Bearer ${token}`);
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({
      totalDepositedUsdt: expect.any(String),
      pendingCount: expect.any(Number),
    });
  });

  it('GET /api/admin/v1/deposits/:id returns 404 for unknown id', async () => {
    const token = await staffToken(app!, UserRoleCode.SUPER_ADMIN);
    const res = await request(app!.getHttpServer())
      .get('/api/admin/v1/deposits/00000000-0000-0000-0000-000000000000')
      .set('Authorization', `Bearer ${token}`);
