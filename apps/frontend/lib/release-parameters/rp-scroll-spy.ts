...
export const RP_SCROLL_OFFSET_PX = 112;
const BOTTOM_THRESHOLD_PX = 64;

export function resolveRpScrollRoot(node: HTMLElement | null): HTMLElement | null {
  if (!node) return null;
...
export function resolveRpActiveSection(scrollRoot: HTMLElement): string {
  const sectionIds = RELEASE_PARAMETERS_IN_PAGE_NAV.map((item) => item.id);
  const sections = sectionIds
    .map((id) => document.getElementById(id))
    .filter((node): node is HTMLElement => node != null);

  if (sections.length === 0) return sectionIds[0] ?? "rp-top";
  ...
  const marker = scrollTop + RP_SCROLL_OFFSET_PX;
...
export function useRpScrollSpy(anchorRef: RefObject<HTMLElement | null>) {
  const [active, setActive] = useState<string>(RELEASE_PARAMETERS_IN_PAGE_NAV[0]?.id ?? "rp-top");

  const syncActive = useCallback(() => {
    const scrollRoot = resolveRpScrollRoot(anchorRef.current);
    if (!scrollRoot) return;
    setActive((prev) => {
      const next = resolveRpActiveSection(scrollRoot);
  ...
  const scrollToSection = useCallback(
    (id: string) => {
      ...
      const top = Math.max(0, sectionTopInRoot(section, scrollRoot) - RP_SCROLL_OFFSET_PX + 8);
