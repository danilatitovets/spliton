...
const landingPath = path.join(__dirname, "../components/dashboard/dashboard-main-landing.tsx");
const pagePath = path.join(__dirname, "../app/app/page.tsx");
...
/** Client landing body — avoids SSR/client HTML mismatches from motion/Intl on /app. */
export function DashboardMainLanding() {
  return (
    <LandingScrollRoot>
      <div
...
const DashboardMainLanding = dynamic(
  () =>
    import("@/components/dashboard/dashboard-main-landing").then((m) => m.DashboardMainLanding),
  {
    ssr: false,
    loading: () => (
      <div className="flex min-h-dvh w-full items-center justify-center bg-black text-sm text-zinc-500">
        Spliton
      </div>
    ),
  },
...
export default function DashboardMainPage() {
  return <DashboardMainLanding />;
}
`,
  "utf8",
);

console.log("landing client-only shell ready");
