...
const TILES = [
  {
    ...
    titleKey: "referral.promo.catalog.title",
    subtitleKey: "referral.promo.catalog.subtitle",
    ctaKey: "referral.promo.catalog.cta",
    href: ROUTES.dashboardCatalog,
  },
  {
    id: "calculator",
    image: "/images/partner-programtab=about/3.jpg",
    titleKey: "referral.promo.calculator.title",
    subtitleKey: "referral.promo.calculator.subtitle",
    ctaKey: "referral.promo.calculator.cta",
    href: ROUTES.calculator,
  },
] as const;

export function ReferralPromoTiles({ className }: { className?: string }) {
  const { t } = useI18n();

  return (
    <section className={cn("grid gap-4 lg:grid-cols-2", className)} aria-label={t("referral.promo.aria")}>
      {TILES.map((tile) => (
        <article
          key={tile.id}
