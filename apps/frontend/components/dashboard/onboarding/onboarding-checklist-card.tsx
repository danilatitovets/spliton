    ...
    return (
      <section className={cn("space-y-4 pb-2", className)} aria-busy="true">
        <div className="flex animate-pulse items-center justify-between gap-4">
          <div className="h-7 w-52 max-w-full rounded bg-white/5" />
  ...
  return (
    <section className={cn("space-y-4 pb-2", className)} aria-label={t("dashboard.onboarding.ariaLabel")}>
      <div className="flex items-center justify-between gap-4">
        ...
        <p className="font-mono text-sm font-medium tabular-nums text-[#d0d6e0]">{progressPct}%</p>
      </div>
      <div className="h-1.5 overflow-hidden rounded-full bg-white/5">
        <div
          className="h-full rounded-full bg-[#3fe280] transition-all duration-500"
